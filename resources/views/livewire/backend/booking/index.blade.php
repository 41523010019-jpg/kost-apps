                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                ...
                                @if ($booking->status === 'expired')
                                    <span class="inline-block ml-2 px-2 py-1 text-xs font-semibold text-white bg-red-500 rounded-full">
                                        Nonaktif
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm">
                                @if ($booking->status === 'expired')
                                    <span class="text-gray-500 text-xs italic">Pembayaran dihentikan</span>
                                @elseif ($bill->status === 'paid')
                                    <p class="text-xs text-green-600 font-semibold">Settlement</p>
                                @elseif (!$bill->payment)
                                    <button wire:click="generateSnapToken({{ $bill->id }})"
                                ...
                                @else
                                    ...
                                    @if ($bill->payment->method === 'midtrans')
                                        @if ($status === 'pending')
                                            <button wire:click="$dispatch('payAgain', { token: '{{ $bill->payment->snap_token }}' })"
                                                class="text-blue-600 text-xs underline">
                                                Bayar Sekarang
                                            </button>
                                ...
                                @else
                                    ...
                                    @role('admin')
                                        @if ($bill->payment && $bill->payment->method === 'cash' && $bill->payment->transaction_status === 'pending')
                                            <button wire:click="markAsPaidCash({{ $bill->id }})"
